...
def build_live_route_template(
    ...
    current_place = get_or_fetch_place(current_location)
    pickup_place = get_or_fetch_place(pickup_location)
    dropoff_place = get_or_fetch_place(dropoff_location)
    ...
    return RouteTemplate(
        provider="osrm",
        notes="Route geometry and travel estimates come from cached geocoding and OSRM routing.",
...
def get_or_fetch_place(query: str) -> ResolvedPlace:
    ...
    url = f"{settings.GEOCODER_BASE_URL}/search?{params}"
    ...
    place = PlaceCache.objects.create(
        ...
        query=query,
        formatted_address=first_result["display_name"][:255],
...
def get_or_fetch_route(origin: ResolvedPlace, destination: ResolvedPlace, label: str) -> RouteLeg:
    ...
    origin_cache = PlaceCache.objects.get(normalized_query=normalize_query(origin.query))
    destination_cache = PlaceCache.objects.get(normalized_query=normalize_query(destination.query))
...
def _merge_geometry(first_coordinates: tuple[tuple[float, float], ...], second_coordinates: tuple[tuple[float, float], ...]) -> list[tuple[float, float]]:
