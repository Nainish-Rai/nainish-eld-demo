...
class TripPlanApiTests(TestCase):
    ...
    def test_plan_trip_creates_persisted_request_and_returns_live_hos_plan(self):
    ...
    def test_plan_trip_rejects_cycle_hours_above_limit(self):
        response = self.client.post(
            reverse("plan-trip"),
            {
                "current_location": "Chicago, IL",
    ...
    def test_get_trip_returns_saved_record(self):
...
class HosPlannerTests(TestCase):
    def test_standard_route_inserts_30_minute_break_after_eight_hours_of_driving(self):
        plan = build_trip_plan(_trip_input())
    ...
    def test_long_drive_spills_into_second_shift_after_ten_hour_rest(self):
        long_route = RouteTemplate(
            ...
            notes="test route",
            legs=(
                RouteLeg("Drive to pickup", "current_location", "pickup_location", 60, Decimal("80.0")),
                RouteLeg("Drive to dropoff", "pickup_location", "dropoff_location", 720, Decimal("640.0")),
            ),
        )
    ...
    def test_cycle_exhaustion_triggers_restart_warning_and_restart_event(self):
        plan = build_trip_plan(_trip_input(current_cycle_used_hours=Decimal("69.00")))
